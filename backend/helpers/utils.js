...
let utils = {
    ...
    getFileExtension(filename) {
        ...
        // If '.' is not found or it's the last character, return an empty string
    ...
    getCurrentTimeOnly(ms, format = "HH:MM AMPM", type = '12') {
    ...
    getCurrentTimeStamp(ms = undefined) {
    ...
    getCurrentTime(ms = undefined) {
        ...
        // get format like this 03_Apr_2024_05_08_pm, with month and am or pm

        let month = data.toLocaleString('default', { month: 'short' });
        let am_pm = hh >= 12 ? 'pm' : 'am'
        let hours = hh > 12 ? hh - 12 : hh

        const D = [dd, month, yyyy, hours, min, am_pm].join('_')
    ...
    excelToJSON(file) {
        ...
        // Initialize an object to store JSON data
        const jsonData = {};
        ...
        workbook.SheetNames.forEach(sheetName => {
            ...
            // Store the JSON data in the object with sheet name as key
    ...
    check_task_upload_data(sheets) {

        let is_valid = false
        if (!sheets || !Object.keys(sheets).length) is_valid = false

        let project_sheet = sheets[Object.keys(sheets)[0]]

        if (!project_sheet || !project_sheet.length) is_valid = false

        let project_sheet_keys = Object.keys(project_sheet[0]).map(key => String(key).split(' ').join('').trim().toLocaleLowerCase())

        if (
            !project_sheet_keys.includes('taskname')
            || !project_sheet_keys.includes('workcategory')
            || !project_sheet_keys.includes('startdate')
            || !project_sheet_keys.includes('enddate')
            || !project_sheet_keys.includes('description')
        ) is_valid = false
    ...
    TimestampToDateTime(ms) {
        ...
        const DateTime = date.toLocaleString('en-IN', options);
        return DateTime;
    },
    getDurationStartEnd() {
        ...
        const weekEnd = new Date(now.getFullYear(), now.getMonth(), now.getDate() + (6 - now.getDay()), 23, 59, 59, 999).getTime();
    ...
    getDaysBetweenDates(start_date, end_date) {

        // return 0 if no days between dates
    ...
    excelToDate(excelDate) {
        ...
        if (String(excelDate).split('-').length == 3) return excelDate
    ...
    async synchronousLoop(data, processData, done) {

        if (data.length > 0) {

            let loop = async (data, i, processData, done) => {

                return await processData(data[i], i, async () => {

                    if (++i < data.length) return await loop(data, i, processData, done)
                    else {
            ...
            return await loop(data, 0, processData, done)
    ...
    password_gen: function () {
        ...
        const numChar = Num.split('').some(num => { return retVal.includes(num) ? true : false; })
        const lowchar = low.split('').some(low => { return retVal.includes(low) ? true : false; })
        const upperChar = upperChars.split('').some(upperChar => { return retVal.includes(upperChar) ? true : false; })

        const splChar = specialChars.split('').some(specialChar => { return retVal.includes(specialChar) ? true : false; })

        splChar ? retVal : retVal += specialChars[Math.floor(Math.random() * specialChars.length)];
        upperChar ? retVal : retVal += upperChars[Math.floor(Math.random() * upperChars.length)]
        numChar ? retVal : retVal += Num[Math.floor(Math.random() * Num.length)]
        lowchar ? retVal : retVal += low[Math.floor(Math.random() * low.length)]

        return retVal;
    },

    convertTime12to24: function (time12h) {
        // time12h format - 09:00 AM or 10:00 PM
    ...
    convertTime12to24bySlots: function (slot) {
        // Slot format - 09:00am - 11:00pm
        let timeSlots = []
        ...
        slots.map(i => {
            timeSlots.push(this.convertTime12to24(i.trim().toUpperCase()))
        })
        console.log(timeSlots);
        return timeSlots
    },

    getDatesInRange: async function (startDate, endDate) {
        ...
        return dates;
    }, genMulterLikeFileFromBase64: async (base64String, originalFilename) => {
        let createMulterFile = (base64String, originalFilename) => {
            const base64Data = base64String.replace(/^data:([a-zA-Z0-9]+\/[a-zA-Z0-9]+);base64,/, '');
            const fileBuffer = Buffer.from(base64Data, 'base64');
            const contentType = extractContentType(base64String);
        ...
        let extractContentType = (base64String) => {
            const typeRegex = /^data:([a-zA-Z0-9]+\/[a-zA-Z0-9]+);base64,/;
            const match = base64String.match(typeRegex);
        ...
        return createMulterFile(base64String, originalFilename);
    ...
    shortenedUUID: () => {
        ...
        const shortenedUUID = uuid.substring(0, 9); // Take the first 10 characters of the UUID
        return `ID_${shortenedUUID}`;
    }
